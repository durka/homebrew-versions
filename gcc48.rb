...
class CurlNoEPSVDownloadStrategy < CurlDownloadStrategy
  def _fetch
    # cURL hangs unless --disable-epsv is passed
    curl @url, '--disable-epsv', '--output', @tarball_path
  end
end
...
class Gcc48 < Formula
  ...
  url 'ftp://gcc.gnu.org/pub/gcc/releases/gcc-4.8.0/gcc-4.8.0.tar.bz2',
    :using => CurlNoEPSVDownloadStrategy
